...
//--- if  new bar set, calculate    
    if(rCount!=rates_total)
      {
       PlotIndexSetInteger(0,PLOT_DRAW_BEGIN,rates_total-Shift-ExChPeriod-1);
       PlotIndexSetInteger(1,PLOT_DRAW_BEGIN,rates_total-Shift-ExChPeriod-1);
       PlotIndexSetInteger(2,PLOT_DRAW_BEGIN,rates_total-Shift-ExChPeriod-1);
       PlotIndexSetInteger(3,PLOT_DRAW_BEGIN,rates_total-Shift-ExChPeriod-1);
       PlotIndexSetInteger(4,PLOT_DRAW_BEGIN,rates_total-Shift-ExChPeriod-1);
//--- calculate coefficient a and b of equation linear regression 
       ...
       sumX2=0.0;
       X=0;
       for(int i=rates_total-Shift-1-ExChPeriod;i<rates_total-Shift-1;i++)
         {
          sumX+=X;
          sumY+=price[i];
          sumXY+=X*price[i];
...
//--- calculate values of main line and error F
       X=0;
       for(int i=rates_total-Shift-1-ExChPeriod;i<rates_total-Shift;i++)
         {
          rlBuffer[i]=b+a*X;
